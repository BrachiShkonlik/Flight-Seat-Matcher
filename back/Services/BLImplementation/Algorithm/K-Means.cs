...
public static class KMeans
{
    public static int[] Cluster(double[,] rawData, int numClusters)
    {
        ...
        double[,] means = Allocate(numClusters, data.GetLength(0)); // small convenience
    ...
    private static double[,] Normalized(double[,] rawData)
    {
        ...
        double[,] result = new double[rawData.GetLength(0), rawData.GetLength(1)];
        ...
        for (int i = 0; i < rawData.GetLength(0); ++i)
        {
            for (int j = 0; j < rawData.GetLength(1); j++)
            {
                ...
                Array.Copy(rawData[i, j], result[i, j], rawData[i, j].Length);
            }
        }
